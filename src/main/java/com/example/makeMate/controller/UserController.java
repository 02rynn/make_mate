...
//                     return userDTO1;
	...
	@PostMapping("/login")
	public UserEntity login(@RequestBody LoginForm loginForm, HttpServletRequest req, HttpServletResponse resp) {
...
//비밀번호 변경
	...
	@ResponseBody
	public void update_password(@RequestBody PasswordDTO passwordDTO, @RequestParam String loginId) {
		log.info("기존비번 {}, 비번체크 {}, 새비번{}, 아이디{}", passwordDTO.getOldPassword(), passwordDTO.getPasswordCheck(),
				passwordDTO.getNewPassword(), loginId);
	...
	@ResponseBody
	public void update_email(@RequestBody EmailDTO emailDTO, @RequestParam String loginId) {
		log.info("새 이메일 {}, 비밀번호 {} 로그인 아이디{}", emailDTO.getNewEmail(), emailDTO.getPassword(), loginId);
		// 이메일이 기존이메일과 일치하면 안됨
